...
download_and_extract_backhaul() {
    ...
    sleep 1
# curl -sSL -o "$DOWNLOAD_DIR/backhaul.tar.gz" "$DOWNLOAD_URL"  # Token-related code commented out
...
# SERVER_COUNTRY=$(curl -sS "http://ipwhois.app/json/$SERVER_IP" | jq -r '.country')  # Token-related code commented out

# Fetch server isp 
# SERVER_ISP=$(curl -sS "http://ipwhois.app/json/$SERVER_IP" | jq -r '.isp')  # Token-related code commented out
...
# # Security Token  # Token-related code commented out
# echo -ne "[-] Security Token (press enter to use default value): "  # Token-related code commented out
# read -r token  # Token-related code commented out
# token="${token:-your_token}"  # Token-related code commented out
...
# token = "${token}"  # Token-related code commented out
keepalive_period = 75
nodelay = ${nodelay}
channel_size = 2048
heartbeat = 40
...
# # Validate and process port mappings  # Token-related code commented out
...
# curl -s -o "$tmp_file" "$url"  # Token-related code commented out
...
# curl -s -L -o "$DEST_DIR/$BACKHAUL_SCRIPT" "$SCRIPT_URL"  # Token-related code commented out
